...
func testPackageToManyTusers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, tuserDBTypes, false, tuserColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, tuserDBTypes, false, tuserColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadTusers(ctx, tx, false, (*[]*Package)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Tusers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Tusers = nil
	if err = a.L.LoadTusers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Tusers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testPackageToManyAddOpTusers(t *testing.T) {
	var err error
	...
	foreigners := []*Tuser{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, tuserDBTypes, false, strmangle.SetComplement(tuserPrimaryKeyColumns, tuserColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Tuser{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddTusers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Tusers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Tusers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Tusers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	packageDBTypes = map[string]string{`ID`: `INTEGER`, `Name`: `TEXT`, `Duration`: `INTEGER`, `Price`: `INTEGER`, `TrafficAllowed`: `FLOAT`, `ResetMode`: `TEXT`, `Active`: `BOOLEAN`}
	_              = bytes.MinRead
)
...
func testPackagesSliceUpdateAll(t *testing.T) {
	...
	if strmangle.StringSliceMatch(packageAllColumns, packagePrimaryKeyColumns) {
		...
		fields = strmangle.SetComplement(fields, packageGeneratedColumns)
	}
